...
@section('content')
    ...
    @include('admin.category._create_edit')
@endsection
    ...
    <script>
        $( document ).ready(function() {
                ...
                columns: [

                    {
                        data:'id',orderable: false, render:function (data,type,full){
                            return `<input class="uk-checkbox checkboxCategory" value="${data}" type="checkbox">`;}
                    },
                    {data: 'title', name: 'title', className: ' uk-text-left'  },
                    {data: 'grades', name: 'grades', className: ' uk-text-left'  },
                    {data: 'subcategories', name: 'subcategories', className: ' uk-text-left'  },
                    ...
                    {data: 'id',className:'uk-text-nowrap',render:function (data,type,full){
                            return (
                                ...
                                '<a  class="dropdown-item edit_row" data-id="'+data+'" data-name="'+full["title"]+'" data-gradeids="'+full["grade_ids"]+'" data-bs-toggle="modal" data-bs-target="#modal-category">' +
            ...
            $(document).on('click','.add-new',function (e){
                $('#titleModalLabel').text("add Category")
                $('#modal-category input').val(null)
                $('#modal-category select').val(null).trigger('change')
                $('#modal-category #type').val("add")
            })

            $(document).on('click','.edit_row',function (e){
                $('#titleModalLabel').text("edit Category")

                $('#modal-category input').val(null)
                $('#modal-category select').val(null).trigger('change')

                $('#modal-category #type').val("edit")
                $('#modal-category #id').val($(this).data('id'))
                $('#modal-category #title').val($(this).data('name'))
                $('#modal-category #grade_id').val(JSON.parse('['+$(this).data('gradeids')+']')).change()
                // $('#modal-category #grade_id').select2("val", JSON.parse('['+$(this).data('gradeids')+']'))
                // console.log(JSON.parse('['+$(this).data('gradeids')+']'));
            ...
            $(document).on('click','#save_category',function (e){
                ...
                var formdata= {
                    name: $('#title').val(),
                    grade_ids: JSON.stringify($('#grade_id').val())
                }
                let page = $('#table_page').val()

                if($('#type').val() == 'add'){
                    create_modal(formdata,'/admin/category/create','POST','modal-user',main_table)
                }
                else if($('#type').val() == 'edit'){
                    // formdata.id= $('#id').val(),
                    create_modal(formdata,'/admin/category/edit/'+$('#id').val(),'POST','modal-user',main_table)
                }
            ...
            $(document).on('click','.SelectorToDelete',function (e){
                e.preventDefault();
                var selectids = [];
                $('.checkboxCategory').each(function (loop) {
                    this.checked ? selectids.push($(this).val()) :null;
                });
                if(selectids.length>0){
                    var url='/admin/delete-all-category'
            ...
            $(document).on('click','#delete_device',function (){
                ...
                $.ajax({
                    ...
                    data: {index: index,category_id:category_id},
                    success: function (res) {
                        if(res.status){
                            main_table.ajax.reload();
                            Swal.fire("Done", "Record delete successful.", "success");
        ...
        $('#setIdCategory').click(function(){
            categoryId();
        })

        function categoryId(){
            if($('#setIdCategory').is(':checked')){
