...
@section('content')
    <!-- classrooms list start -->
    <section class="app-classroom-list">
        ...
        <div class="card">
            ...
            <div class="card-datatable table-responsive pt-0">
                <table class="classroom-list-table table" id="classroom_table">
            ...
            @include('admin.classroom._create_edit')
            @include('admin.classroom._room_demo')
        </div>
        <!-- list and filter end -->
    </section>
    <!-- classrooms list ends -->
@endsection
...
@section('page-script')
    ...
    <script>
        $( document ).ready(function() {
            var classroom_table
            classroom_table =  $('#classroom_table').DataTable({
                processing: true,
                serverSide: true,
                ...
                dom:
                    ...
                    '<"col-sm-12 col-lg-2 row d-flex justify-content-center justify-content-lg-start"  l>' +
                    '<"col-sm-12 col-lg-2 d-flex justify-content-center justify-content-lg-start" <"demoroom">>' +
                ...
                ajax: {
                    ...
                    beforeSend: function () {
                        $('#table_page').val($('#classroom_table').DataTable().page())
                    },
                },
                columns: [
                    ...
                    {data: 'title' , className: ' uk-text-left' ,
                        render:function (data,type,full){
                            return `<a class="link position-relative" href="{{route('admin.classroom.show','')}}/${full['id']}">
                                ${full['demo'] ? '<span class="badge rounded-pill badge-light-success me-1 demo_room">DEMO</span>':''}
                                ${data}
                            </a> `
                        }
                    },
                    {data: 'subject', name: 'enroll_class', className: ' uk-text-left'  },
                    {data: 'grade', name: 'enroll_class', className: ' uk-text-left'  },
                    {data: 'instructor', name: 'created_at', className: ' uk-text-left',
                        render:function (data,type,full){
                            return `<a class="link" href="/u/${full['name_id']}">${data}</a> `
                        }
                    },
                    {data: 'students', name: 'enroll_class', className: ' uk-text-left'  },
                    {data: 'id',className:'uk-text-nowrap',render:function (data,type,full){
                            return (
                                ...
                                `<a href="{{route('admin.classroom.show','')}}/${data}"  id="edit_row" class="dropdown-item">` +
                                feather.icons['eye'].toSvg({ class: 'font-small-4 me-50' }) +
                                'Show</a>' +
                                `<a href="{{route('admin.classrooms.edit','')}}/${data}?page_tbl=${$('#table_page').val()}"  id="edit_row" class="dropdown-item">` +
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        attr: {
                            ...
                            'data-bs-target': '#modal-classroom'
                        },
                        init: function (api, node, config) {
                            $(node).removeClass('btn-secondary');
                        }
            ...
            if ('{{session()->get("page_tbl")}}') {
                setTimeout(() => {
                    $('#classroom_table').DataTable().page(parseInt('{{session()->get("page_tbl")}}')).draw("page");
                }, 200);
            }
            ...
            $(document).on('click','#save_demo_class_room',function (e){
                ...
                create_modal(formdata,'{{route("admin.classrooms.setdemo")}}','POST','modal-classroom',classroom_table);
            });
            ...
            $(document).on('click','#save_classroom',function (e){
                ...
                create_modal(formdata,'{{route("admin.classrooms.create")}}','POST','modal-classroom',classroom_table);
            });
            ...
            $(document).on('click','#delete_row',function (e){
                ...
                let url = '{{route("admin.classrooms.delete","")}}/'+rowID
                deleteRow(rowID,url)
                    ...
                    if (result.isConfirmed){
                        $.ajax({
                            ...
                            success: function (res) {
                                ...
                                classroom_table.ajax.reload();
                                page ? $('#classroom_table').DataTable().page(parseInt(page)).draw("page") :null
