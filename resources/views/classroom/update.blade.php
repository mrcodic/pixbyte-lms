...
<div class="wrapper-page-light f-height">
    ...
    <div class="header-wrap page-dark">
        <div class="uk-container uk-container-expand rm-expand">
            ...
            <div class="page-header uk-margin-medium-top uk-margin-medium-bottom breadcrumb">
                <h3 class="uk-margin-remove-bottom title-add">Edit Classroom</h3>
    ...
    <div class="uk-container uk-container-expand rm-expand uk-margin-medium-top uk-margin-left">
        ...
        <div class="add-classroom">
            <form id="target" action="{{ route('classrooms.update',['classroom'=>$class->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <fieldset class="uk-fieldset add-new" uk-grid>
                    ...
                    <div class="uk-margin uk-width-1-1">
                        <input class="uk-input title @error('title')error-border @enderror" name="title" type="text" placeholder="Classroom title goes here....." value="{{ $class->title}}" autofocus>
                        @error('title')
                        ...
                        <div class="uk-form-controls">
                            <select class="uk-select @error('grade_id')error-border @enderror" id="grade" name="grade_id">
                                ...
                                @foreach ( $grades as $grade )
                                    <option {{$grade_id === $grade->id ? 'selected':' '}} value=" {{$grade->id}} ">{{ $grade->name }}</option>
                                @endforeach
                            </select>
                            @error('grade_id')
                                <span class="error-msg">
                        ...
                        <div class="uk-form-controls">
                            <select class="uk-select @error('room_scheduel')error-border @enderror" id="room-Schedule" name="room_scheduel">
                                ...
                                @foreach ( $schedules as $schedule )
                                    <option {{$class->room_scheduel === $schedule->id ? 'selected':' '}} value=" {{$schedule->id}} ">{{ $schedule->name }}</option>
                                @endforeach
                            </select>
                            @error('room_scheduel')
                                <span class="error-msg">
                    ...
                    <div class="uk-margin uk-width-1-4@m uk-width-1-1@s">
                        ...
                        <div class="uk-form-controls">
                            <input id="absence-rooms" class="uk-input @error('absence_times')error-border @enderror" name="absence_times" type="number" placeholder="Rooms number" value="{{$class->absence_times}}" autofocus>
                            @error('absence_times')
                                <span class="error-msg">
                    ...
                    <div class="uk-margin uk-width-1-1">
                        ...
                        <div class="uk-form-controls">
                            <textarea id="description" class="uk-textarea @error('description')error-border @enderror" rows="5" placeholder="Description" name="description">{{ $class->description }}</textarea>
                            @error('description')
                                <span class="error-msg">
                    ...
                    <div class="uk-margin uk-width-1-4@m uk-width-1-1@s">
                        ...
                        <ul class="edit-room-material" id="parent">
                            @foreach($class->photos  as $key => $me)
                            <li data-id="{{$me->id}}">  <a download="custom-filename.jpg" href="{{url('uploads/media/'.$me->path)}}"> {{substr($me->path,0,30)}} </a>  <span uk-icon="icon: close; ratio: 1"  data-id ="{{$me->id}}" class="deleteImage"> </span></li>
                            @endforeach
                        </ul>
                        <div class="js-upload uk-placeholder uk-text-center">
                            ...
                            <div uk-form-custom>
                                <input type="file">
                                <span class="uk-link " id="fileUpload"  @if($class->photos()->first()) style="display: none" @endif >selecting one</span>
                            </div>
                        </div>
                    ...
                    <div class="uk-margin uk-width-1-4@m uk-width-1-2@s">
                        ...
                        <div class="switcher">
                            <label class="uk-switch " for="setting_missed">
                                <input type="checkbox"  id="setting_missed" @if($class->setting_missed==1) checked @endif  name="setting_missed" >
